export const migration001InitV2 = {
  version: '001_init_v2',
  ...
  async up(db) {
    ...
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_materials_category ON materials(category, subcategory)'
    )
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_variants_material_id ON material_variants(material_id)'
    )
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_works_category ON works(category)'
    )
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_work_tiers_work_id ON work_price_tiers(work_id)'
    )
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_system_layers_system_id ON system_layers(system_id)'
    )
    await db.execute(
      'CREATE INDEX IF NOT EXISTS idx_system_params_system_id ON system_params(system_id)'
    )
    await db.execute(
