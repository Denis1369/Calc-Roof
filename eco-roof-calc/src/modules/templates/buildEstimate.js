...
import { normalizeSelectedTemplateOptionKeys } from '@/modules/templates/templateEnhancements'
...
function buildSections({
  ...
  const activeLayers = layers.filter((layer) => {
    ...
    return selectedSet.has(normalizeOptionKey(layer.feature_code))
  })
  ...
  const sections = activeLayers.map((layer) => {
    ...
    return {
      ...
      featureCode: normalizeOptionKey(layer.feature_code || ''),
      works,
      materials
    }
  })
